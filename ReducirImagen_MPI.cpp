//mpic++ -std=c++11 ReducirImagen_MPI.cpp `pkg-config --libs --cflags opencv` -lpthread -lm -o program
...
void reduccion720(int threadId, int THREADS, int **imgR, int **imgG, int **imgB, int **outR, int **outG, int **outB)
{
...
void sumarMatrices(int rows, int cols, int **outRAux, int **outGAux, int **outBAux)
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            outR[i][j] += outRAux[i][j];
            outG[i][j] += outGAux[i][j];
            outB[i][j] += outBAux[i][j];
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    outRAux = new int *[outRows];
    for (size_t i = 0; i < outRows; ++i)
        outRAux[i] = new int[outCols];

    outGAux = new int *[outRows];
    for (size_t i = 0; i < outRows; ++i)
        outGAux[i] = new int[outCols];

    outBAux = new int *[outRows];
    for (size_t i = 0; i < outRows; ++i)
        outBAux[i] = new int[outCols];
